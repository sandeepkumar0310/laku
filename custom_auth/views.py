...
class Dashboard(View):
    ...
    def post(self,request):
        ...
        if form.is_valid():
            ...
            post.save()
            return redirect('/dashboard/')  # Redirect to post detail page
...
def save_like(request):
    ...
    print(type_like,'ooooooooo')
    if type_like == "like":
        print("DFSFF")
        pass
        user                = LikeOnBlog.objects.create(user_id = request.user.id, blog_id = blog_id)
    else:
        user                = LikeOnBlog.objects.filter(user_id = request.user.id, blog_id = blog_id).delete()
...
def share_blog(request):
    ...
    print(email,blog_id,'lllllsssssssssssssssssssssssssssssssssssssl')
    ...
    # send_mail(subject, html_message, sender_email, recipient_list)
